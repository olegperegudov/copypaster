...
//! history goes through here instead, and the mode is set at creation rather
...
mod tests {
    ...
    fn a_written_file_is_readable_by_its_owner_and_nobody_else() {
        let dir = std::env::temp_dir().join(format!("cp-private-{}", std::process::id()));
        create_dir(&dir).unwrap();
        let f = dir.join("secret");
        write(&f, b"password").unwrap();
        assert_eq!(mode(&f), 0o600, "the clipboard history must not be world-readable");
    ...
    fn rewriting_a_file_does_not_widen_it() {
        let dir = std::env::temp_dir().join(format!("cp-private-rw-{}", std::process::id()));
        create_dir(&dir).unwrap();
        let f = dir.join("index.json");
